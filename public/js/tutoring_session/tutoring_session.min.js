...
b.removeClass("hide")},tsDownloadFile=function(a){window.open("/ajax/session/download_file?file="+a)},tsRemoveFile=function(a){$('.ts-file-manager-file-actions[data-ss-file="'+a+'"]').parents(".ts-file-manager-file").remove()};$(document).on("mouseenter",".ts-file-manager-file",function(){$(".ts-file-manager-file-actions",this).fadeIn()}).on("mouseleave",".ts-file-manager-file",function(){$(".ts-file-manager-file-actions",this).fadeOut()});$(document).on("click",".ts-file-manager-file-action",function(){var a=this.getAttribute("data-ss-action"),b=$(this).parent().data("ss-file");"download"==a?tsDownloadFile(b):"remove"==a&&(tsRemoveFile(b),$.ajax({url:"/ajax/session/remove_file",type:"POST",data:{file:b}}).done(function(a){}))});
$(document).on("change",'input[name="ts-file-upload"]',function(){var a=new XMLHttpRequest,b=new FormData;b.append("ts-file",this.files[0]);a.open("POST","/ajax/session/receive_file",!0);a.send(b);a.onreadystatechange=function(){4==a.readyState&&200==a.status&&JSON.parse(a.responseText)}});$("#ss-modal-ts-finished-stars").on("click",".ss-modal-ts-finished-star",function(){var a=$(".ss-modal-ts-finished-star").index(this);$(".ss-modal-ts-finished-star").attr("src","/images/rating_empty.png");$(".ss-modal-ts-finished-star").slice(0,a+1).attr("src","/images/rating_full.png");$(this).parent()[0].setAttribute("data-ss-stars-amount",a+1)}).on("mouseenter",".ss-modal-ts-finished-star",function(){var a=$(".ss-modal-ts-finished-star").index(this);$(".ss-modal-ts-finished-star").attr("src","/images/rating_empty.png");
