$(function(){$(".header-notice").html("Please wait ...")});var session_finished=!1,waiting_interval=!1,ts_socket="local"===environment?io.connect("http://studysquare.lh:53101"):io.connect("http://studysquare.com:53101");
...
1E3,a.finish_time)});var sourcevid=document.getElementById("webrtc-source"),remotevid=document.getElementById("webrtc-remote"),localStream=null,peerConn=null,mediaConstraints={mandatory:{OfferToReceiveAudio:!0,OfferToReceiveVideo:!0}},_recorder=null;
function startVideo(){if(localStream)return!0;navigator.getUserMedia=navigator.getUserMedia||navigator.webkitGetUserMedia||window.navigator.mozGetUserMedia||navigator.msGetUserMedia;window.URL=window.URL||window.webkitURL;window.AudioContext=window.AudioContext||window.webkitAudioContext;navigator.getUserMedia({video:!0,audio:!0},function(a){localStream=a;a=(new AudioContext).createMediaStreamSource(a);_recorder=new Recorder(a);_recorder.record();sourcevid.mozSrcObject?sourcevid.mozSrcObject=localStream:
(sourcevid.src=window.URL.createObjectURL(localStream),sourcevid.muted=!0);sourcevid.play();connect()},function(a){console.log("Error code ["+a.code+"]")})}function toggleVideo(){localStream.getVideoTracks()[0].enabled=!localStream.getVideoTracks()[0].enabled}function toggleAudio(){localStream.getAudioTracks()[0].enabled=!localStream.getAudioTracks()[0].enabled}
...
remotevid.play()},!1);peerConn.addEventListener("removestream",function(a){remotevid.src=""},!1)}function xhr(a,b,d){var c=new XMLHttpRequest;c.onreadystatechange=function(){4==c.readyState&&200==c.status&&d(location.href+c.responseText)};c.open("POST",a);c.send(b)}function end_audio_recording(){_recorder&&_recorder.exportWAV(function(a){_recorder.clear();var b=new FormData;b.append("audio-filename","recording.wav");b.append("audio-blob",a);xhr("/ajax/session/receive_audio",b,function(a){})})}
setInterval(function(){session_finished||end_audio_recording()},6E4);$(".ts-cam-toggle").change(function(){toggleVideo()});$(".ts-mic-toggle").change(function(){toggleAudio()});var tsInitText=function(a){tinymce.init({selector:"textarea#ts-text-textarea",width:718,height:397,resize:!1,statusbar:!1,setup:a,plugins:["advlist autolink lists link image charmap print preview anchor","searchreplace visualblocks fullscreen","insertdatetime media table contextmenu paste"],menu:{file:{title:"File",items:"save_file print"},edit:{title:"Edit",items:"undo redo | cut copy paste | selectall | searchreplace"},insert:{title:"Insert",items:"image link | charmap insertdatetime"},view:{title:"View",
...
$(document).on("click",".ts-chat-send",function(){""!==$.trim($(".ts-chat-textarea").val())&&(tsChatAdd($.trim($(".ts-chat-textarea").val())),ts_socket.emit("tutoring_session_data",{what:"chat",message:$.trim($(".ts-chat-textarea").val())}),$(".ts-chat-textarea").val(""))});var tsNewFile=function(a){var b=$(".snippet-ts-file-manager-file").clone().removeClass("snippet-ts-file-manager-file"),d=12<a.filename.length?a.filename.substr(0,12)+"...":a.filename;$(".ts-file-manager-file-actions",b)[0].setAttribute("data-ss-file",a.filename);$(".ts-file-manager-file-caption",b).text(d);$(".ts-file-manager-file-caption",b).attr("title",a.filename);$(".ts-file-manager-file-icon",b).attr("src",a.icon);$(".ts-file-manager-file-icon",b).attr("alt",a.filename);$(".ts-file-manager-files").append(b);
