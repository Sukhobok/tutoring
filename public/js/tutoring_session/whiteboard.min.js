...
$(document).on("click",".ts-whiteboard-tools-color",function(){var b;b="ts-whiteboard-tools-color-"+this.getAttribute("data-ts-color-name");var a=this.getAttribute("data-ts-color-name"),c=this.getAttribute("data-ts-color"),d;d="ts-whiteboard-tools-color-"+$(".ts-whiteboard-tools-color").first()[0].getAttribute("data-ts-color-name");var g=$(".ts-whiteboard-tools-color").first()[0].getAttribute("data-ts-color-name"),e=$(".ts-whiteboard-tools-color").first()[0].getAttribute("data-ts-color");$(".ts-whiteboard-tools-color").first().removeClass(d).addClass(b);
$(".ts-whiteboard-tools-color").first()[0].setAttribute("data-ts-color-name",a);$(".ts-whiteboard-tools-color").first()[0].setAttribute("data-ts-color",c);$(this).removeClass(b).addClass(d);$(this)[0].setAttribute("data-ts-color-name",g);$(this)[0].setAttribute("data-ts-color",e);styles[0].strokeColor=c;ts_socket.emit("tutoring_session_data",{what:"wb_style",type:"color",value:c})});$(document).on("click",".ts-whiteboard-tools-sizes",function(){36==$(this).width()?$(this).animate({width:233}):$(this).animate({width:36})});
$(document).on("click",".ts-whiteboard-tools-size",function(){var b=parseInt(this.getAttribute("data-ts-size")),a=parseInt($(".ts-whiteboard-tools-size").first()[0].getAttribute("data-ts-size"));$(".ts-whiteboard-tools-size").first()[0].setAttribute("data-ts-size",b);$(".ts-whiteboard-tools-size").first().text(b+"px");this.setAttribute("data-ts-size",a);$(this).text(a+"px");styles[0].strokeWidth=b;ts_socket.emit("tutoring_session_data",{what:"wb_style",type:"size",value:b})});var _current_canvas=document.getElementById("ts-canvas"),base64_to_blob=function(b){var a=b.split(",")[1];b=(b=b.split(",")[0].split(":")[1].split(";")[0])||"";for(var a=atob(a),c=a.length,d=Math.ceil(c/1024),g=Array(d),e=0;e<d;++e){for(var f=1024*e,h=Math.min(f+1024,c),k=Array(h-f),l=0;f<h;++l,++f)k[l]=a[f].charCodeAt(0);g[e]=new Uint8Array(k)}return new Blob(g,{type:b})};
$(document).on("click",".ts-take-screenshot",function(){var b=_current_canvas.toDataURL(),b=base64_to_blob(b),a=new XMLHttpRequest,c=new FormData;c.append("ts-file",b);c.append("ts-file-type","screenshot");a.open("POST","/ajax/session/receive_file",!0);a.send(c);a.onreadystatechange=function(){4==a.readyState&&200==a.status&&JSON.parse(a.responseText)}});ts_socket.on("tutoring_session_data",function(b){console.log(b);if("object"===typeof b)switch(b.what){case "draw":b.e.point=new Point(b.e.point[1],b.e.point[2]);switch(b.tool){case "pencil":switch(b.ev){case "mousedrag":pencilTool.onMouseDrag(b.e,1);break;case "mousedown":pencilTool.onMouseDown(b.e,1);break;case "mouseup":pencilTool.onMouseUp(b.e,1)}break;case "erase":switch(b.ev){case "mousedrag":eraseTool.onMouseDrag(b.e,1);break;case "mousedown":eraseTool.onMouseDown(b.e,1);break;case "mouseup":eraseTool.onMouseUp(b.e,
