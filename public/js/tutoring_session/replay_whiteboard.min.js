...
$(document).on("click",".ts-whiteboard-tools-size",function(){var b=parseInt(this.getAttribute("data-ts-size")),a=parseInt($(".ts-whiteboard-tools-size").first()[0].getAttribute("data-ts-size"));$(".ts-whiteboard-tools-size").first()[0].setAttribute("data-ts-size",b);$(".ts-whiteboard-tools-size").first().text(b+"px");this.setAttribute("data-ts-size",a);$(this).text(a+"px");styles[0].strokeWidth=b;ts_socket.emit("tutoring_session_data",{what:"wb_style",type:"size",value:b})});console.log("test");
var Player=function(b,a,d,e){var c=this;c.start_position=b;c.position=b;c.end_position=a;c.duration=c.end_position-c.start_position;c.looper=null;c.last_item=0;c.play=function(){c.looper=setInterval(function(){for(i=c.position;i<c.position+100;i++)for(;c.last_item!=_data.length-1&&_data[c.last_item].t<=i;){if(_data[c.last_item].t==i){var a=JSON.parse(_data[c.last_item].d);console.log(a);if("object"===typeof a)switch(a.what){case "draw":a.e.point=new Point(a.e.point[1],a.e.point[2]);switch(a.tool){case "pencil":switch(a.ev){case "mousedrag":pencilTool.onMouseDrag(a.e,
1);break;case "mousedown":pencilTool.onMouseDown(a.e,1);break;case "mouseup":pencilTool.onMouseUp(a.e,1)}break;case "erase":switch(a.ev){case "mousedrag":eraseTool.onMouseDrag(a.e,1);break;case "mousedown":eraseTool.onMouseDown(a.e,1);break;case "mouseup":eraseTool.onMouseUp(a.e,1)}break;case "rectangle":switch(a.ev){case "mousedrag":rectangleTool.onMouseDrag(a.e,1);break;case "mousedown":rectangleTool.onMouseDown(a.e,1);break;case "mouseup":rectangleTool.onMouseUp(a.e,1)}break;case "ellipse":switch(a.ev){case "mousedrag":ellipseTool.onMouseDrag(a.e,
1);break;case "mousedown":ellipseTool.onMouseDown(a.e,1);break;case "mouseup":ellipseTool.onMouseUp(a.e,1)}break;default:switch(a.ev){case "mousedrag":lineTool.onMouseDrag(a.e,1);break;case "mousedown":lineTool.onMouseDown(a.e,1);break;case "mouseup":lineTool.onMouseUp(a.e,1)}}project.view.update();break;case "wb_style":switch(a.type){case "color":styles[1].strokeColor=a.value;break;case "size":styles[1].strokeWidth=a.value}break;case "chat":tsChatAdd(a.message);break;case "mode":tsSelectMode(a.mode);
break;case "wb_actions":switch(a.action){case "undo":tsUndo();break;case "redo":tsRedo();break;case "clear":tsClear()}break;case "coding":switch(a.action){case "change_language":tsCodingChangeLang(a.value);break;case "change_template":tsCodingChangeTemplate(a.value);break;case "action":tsCodingAction(a.value);break;case "data":tsCodingEditor.setValue(a.value.code_data),tsCodingEditor.gotoLine(a.value.cursor_at.row+1,a.value.cursor_at.column)}break;case "text":tsTextReceive(a.value)}}c.last_item++}c.position+=
100;c.position>=c.end_position&&c.stop()},100)};c.pause=function(){clearInterval(c.looper)};c.stop=function(){clearInterval(c.looper);c.position=c.start_position}},p=new Player(_started_at,_ended_at,1,1);$("#ss-action-play").click(function(){"ll"==this.innerHTML?(this.innerHTML="&#9658;",p.pause(),$("#action_stop_whiteboard").hide()):(this.innerHTML="ll",p.play(),$("#action_stop_whiteboard").show())});$("#ss-action-stop").click(function(){p.stop();$("#ss-action-stop").hide();$("#ss-action-play").html("&#9658;")});
