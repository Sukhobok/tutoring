  ...
  function ot_type_custom_post_type_select( $args = array() ) {
    ...
    echo '<div class="format-setting type-custom-post-type-select ' . ( $has_desc ? 'has-desc' : 'no-desc' ) . '">';
      ...
      echo '<div class="format-setting-inner">';
		...
		/*Custom to save product info to course*/
		if(isset($post_type[0]) && $post_type[0] == 'product')
		{
			global $post;
			...
			$total_sales = 0;
			 if ( is_array( $my_posts ) && ! empty( $my_posts ) ) {
			  ...
			  foreach( $my_posts as $my_post ) {
				  if( selected( $field_value, $my_post->ID, false )  !== '')
				  {
					$price = get_post_meta( $my_post->ID, '_regular_price', true);
					$total_sales = get_post_meta( $my_post->ID, 'total_sales', true);
				  }
			  }
			...
			if ( ! add_post_meta( $post->ID, '_xproduct_price', $price, true ) ) { 
			   update_post_meta (  $post->ID, '_xproduct_price', $price );
			}
			
			if ( ! add_post_meta( $post->ID, '_xproduct_sales', $total_sales, true ) ) { 
			   update_post_meta (  $post->ID, '_xproduct_sales', $total_sales );
			}
		}
