...
class Recent_Tweets_Widget extends WP_Widget {
		...
		public function __construct() {
			parent::__construct(
				...
				'SpaceX Recent Tweets Widget', 
				array( 'description' => __( 'Display recent tweets', 'spacex' ), )
			);
		}
						...
						echo '<ul class="tweet_list">';
							...
							foreach($tp_twitter_plugin_tweets as $tweet){					
								if(!empty($tweet['text'])){
									...
									echo '<li><span class="tweet_icon fa fa-twitter"></span> <span class="tweet_text">'.tp_convert_links($tweet['text']).'</span><a class="tweet_time" target="_blank" href="http://twitter.com/'.$instance['username'].'/statuses/'.$tweet['status_id'].'">'.tp_relative_time($tweet['created_at']).'</a></li>';
