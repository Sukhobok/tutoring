...
class Events_Widget extends WP_Widget
{
	...
	{
		$widget_ops = array('classname' => 'widget_featured_event', 'description' => __( "Displays events.", "spacex" ) );
		parent::__construct('featured-events', __( 'SpaceX Events', "spacex" ), $widget_ops);
		
		$this-> alt_option_name = 'widget_featured_event';
	...
	function widget($args, $instance)
	{
		...
		$cache = wp_cache_get('widget_featured_event', 'widget');
		...
		extract($args);
		echo $before_widget;
		$title = apply_filters('widget_title', empty($instance['title']) ? __( 'Events', "spacex" ) : $instance['title'], $instance, $this->id_base);
		...
		echo do_shortcode('[xevents featured="'.$instance['featured'].'" latest="yes" perpage="'.$number.'" layout="list" orderby="'.$orderby.'" order="'.$order.'"][/xevents]');
		
		echo $after_widget;
		$cache[$args['widget_id']] = ob_get_flush();
		wp_cache_set('widget_featured_event', $cache, 'widget');
	}
	...
	{
		wp_cache_delete('widget_featured_event', 'widget');
	}
	...
	function form( $instance )
	{
        ...
        <p>
        <input id="<?php echo $this->get_field_id('featured'); ?>" name="<?php echo $this->get_field_name('featured'); ?>" type="checkbox" value="yes" <?php checked( 'yes', $featured ); ?> />
        
        <label for="<?php echo $this->get_field_id('featured'); ?>"><?php echo __('Feature Events', 'spacex'); ?></label>
