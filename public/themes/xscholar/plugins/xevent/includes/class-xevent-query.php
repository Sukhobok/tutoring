...
class XEVENT_QUERY {
	...
	public function pre_get_posts( $q ) {
		...
		if ( ! $q->is_post_type_archive( 'xevent' ) && ! $q->is_tax( get_object_taxonomies( 'xevent' ) ) )
		return;
		...
		$number = ot_get_option('events_per_page');	
		if($number == '')
		{
			$number = 10;
		}
		if( $q->get('post_type') == 'xevent' && is_archive() )
		{
				$q->query_vars['posts_per_page'] = $number; //display all is -1
	
		}
		if( is_search() && $_GET['post_type'] == 'event') {
			
			$q->set( 'post_type', 'xevent' );
			$q->is_archive           = true;
	...
	public function the_posts( $posts, $query = false ) {
		...
		if 	( ! $query->is_post_type_archive( 'xevent' ) && ! $query->is_tax( get_object_taxonomies( 'xevent' ) ) )
	   		return $posts;
	...
	public function product_query( $q ) {
		...
		$q->set( 'posts_per_page', $q->get( 'posts_per_page' ) ? $q->get( 'posts_per_page' ) : apply_filters( 'loop_shop_per_page', xt_get_option( 'posts_per_page' ) ) );
		...
		do_action( 'xevent_product_query', $q, $this );
	}
	...
	public function get_catalog_ordering_args( $orderby = '', $order = '' ) {
		...
		switch ( $orderby ) {
			...
			case 'price' :
				...
				$args['order']    = $order == 'DESC' ? 'DESC' : 'ASC';
				$args['meta_key'] = '_xproduct_price';
			break;
			case 'popularity' :
				$args['orderby']  = "meta_value_num";
				$args['order']    = $order == 'DESC' ? 'DESC' : 'ASC';
				$args['meta_key'] = '_xproduct_sales';
			break;
			case 'rating' :
				$args['orderby']  = "meta_value_num";
				$args['order']    = $order == 'DESC' ? 'DESC' : 'ASC';
				$args['meta_key'] = '_votes_count';
	...
	/**
	 * WP Core doens't let us change the sort direction for invidual orderby params - http://core.trac.wordpress.org/event/17065
	 *
	...
	public function price_filter( $filtered_posts ) {
	    ...
	    if ( isset( $_GET['max_price'] ) && isset( $_GET['min_price'] ) ) {
	        ...
	        $matched_products_query = apply_filters( 'woocommerce_price_filter_results', $wpdb->get_results( $wpdb->prepare("
				...
				WHERE post_type IN ( 'xevent' ) AND post_status = 'publish' AND meta_key = %s AND meta_value BETWEEN %d AND %d
			", '_price', $min, $max ), OBJECT_K ), $min, $max );

	        if ( $matched_products_query ) {
	            foreach ( $matched_products_query as $product ) {
	                if ( $product->post_type == 'xevent' )
