...
class SC_Query {
	...
	public function pre_get_posts( $q ) {
		...
		if ( ! $q->is_post_type_archive( 'xcourse' ) && ! $q->is_tax( get_object_taxonomies( 'xcourse' ) ) )
		return;
		...
		$number = ot_get_option('courses_per_page');	
		if($number == '')
		{
			$number = 10;
		}
		if( $q->get('post_type') == 'xcourse' && is_archive() )
		{
				$q->query_vars['posts_per_page'] = $number; //display all is -1
		}
		
		if( is_search() && $_GET['post_type'] == 'course') {
			
			$q->set( 'post_type', 'xcourse' );
			$q->is_archive           = true;
	...
	public function the_posts( $posts, $query = false ) {
		...
		if 	( ! $query->is_post_type_archive( 'xcourse' ) && ! $query->is_tax( get_object_taxonomies( 'xcourse' ) ) )
	   		return $posts;
	...
	public function product_query( $q ) {
		...
		$q->set( 'posts_per_page', $q->get( 'posts_per_page' ) ? $q->get( 'posts_per_page' ) : apply_filters( 'loop_shop_per_page', sc_get_option( 'posts_per_page' ) ) );
		...
		do_action( 'xcourse_product_query', $q, $this );
	}
	...
	public function price_filter( $filtered_posts ) {
	    ...
	    if ( isset( $_GET['max_price'] ) && isset( $_GET['min_price'] ) ) {
	        ...
	        $matched_products_query = apply_filters( 'woocommerce_price_filter_results', $wpdb->get_results( $wpdb->prepare("
				...
				WHERE post_type IN ( 'xcourse' ) AND post_status = 'publish' AND meta_key = %s AND meta_value BETWEEN %d AND %d
			", '_price', $min, $max ), OBJECT_K ), $min, $max );

	        if ( $matched_products_query ) {
	            foreach ( $matched_products_query as $product ) {
	                if ( $product->post_type == 'xcourse' )
